...
#include "caffe/layers/mask_outputs_layer.hpp"

namespace caffe {

template <typename Dtype>
void MaskOutputsLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void MaskOutputsLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  int kernel_size = this->layer_param_.mask_outputs_param().kernel_size();
  const int num = bottom[0]->num();
  const int height = bottom[0]->height();
